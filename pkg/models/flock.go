...
type Flock struct {
	...
	UserID               uint            `json:"user_id" gorm:"index;not null"` // Multitenancy support
	...
	MortalityRateData    []float64       `json:"mortality_rate_data" gorm:"type:json"`
	...
	VaccinationSchedule  []Vaccination   `json:"vaccination_schedule" gorm:"type:json"`
...
// Vaccination represents vaccination records
...
// BeforeCreate GORM hook to handle JSON serialization
func (f *Flock) BeforeCreate(tx *gorm.DB) error {
	return nil
}

// BeforeUpdate GORM hook to handle JSON serialization
